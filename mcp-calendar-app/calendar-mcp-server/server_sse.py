#!/usr/bin/env python3
"""
HTTP/SSE MCP Calendar Server using FastMCP.
This provides HTTP access to the Calendar MCP server.
"""
...
@mcp.tool()
async def get_all_events(category: str = None, status: str = None) -> str:
    """Get all events/schedules from the Redwood Digital University calendar.

    Args:
        category: Filter by event category (optional): Lecture, Lab, Meeting, Office Hours, Assignment, Defense, Workshop, Study Group, Seminar, Grading, Advising
        status: Filter by completion status (optional): not_started, in_progress, completed
    """
...
async def create_event(
    ...
    Args:
        ...
        category: Event category (Lecture, Lab, Meeting, Office Hours, Assignment, Defense, Workshop, Study Group, Seminar, Grading, Advising)
...
@mcp.tool()
async def get_upcoming_events(days: int = 7, category: str = None) -> str:
...
@mcp.tool()
async def get_calendar_statistics(period: str = "month") -> str:
    """Get calendar statistics and overview.

    Args:
        period: Time period for statistics (week, month, semester)
    """
    all_events = await make_calendar_api_request("GET", "/schedules")
...
if __name__ == "__main__":
    logger.info("🎓 Starting Redwood Digital University Calendar MCP Server (SSE mode)")
    logger.info(f"📡 Calendar API URL: {CALENDAR_API_BASE_URL}")
    logger.info(f"🔄 Server starting on {MCP_HOST}:{MCP_PORT}...")
